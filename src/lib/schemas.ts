...
export const LLMNoteBlockSchema = z.object({
  ...
  summary: z
    .string()
    .describe('Markdown formatted summary of the content. Use bolding for key terms.'),
  visualType: z.string().describe('The type of visual aid to generate for this block'),
  d2Code: z
    .string()
    .optional()
    .describe(
      "Valid D2 diagram code if visualType is 'diagram'. MUST NOT include markdown code fences."
    ),
  imageQuery: z
    .string()
    .optional()
    .describe("Search query for a stock photo if visualType is 'icon' or fallback"),
...
export const NoteBlockSchema = z.object({
  ...
  imageQuery: z
    ...
    .describe("Search query for a stock photo if visualType is 'icon' or fallback"),
  blockType: z
    .enum(['content', 'deep-dive'])
    .default('content')
    .describe('Type of block: original content or AI-generated deep dive'),
  deepDiveMode: z
    .enum(['eli5', 'analogy', 'mental-model'])
    .optional()
    .describe("Deep dive explanation mode if blockType is 'deep-dive'"),
  isStreaming: z
    .boolean()
    .optional()
    .describe('Whether the block content is currently being streamed'),
  renderedSvg: z.string().optional().describe('Cached SVG output from D2 rendering'),
});
...
export const LLMNoteSchema = z.object({
  blocks: z
    .array(LLMNoteBlockSchema)
    .describe('A list of note blocks representing the study material'),
});
